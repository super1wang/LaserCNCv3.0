...
承接 C2a `2259c5a`，C2b1–3 已通过本地实现检查点，最终签核仍须 ST1D。只完善内核准入、生命周期、持久身份和失效协议，不实现 CAD/CAM、Machine/Process、产品 CLI/RPC 或 GUI；下一步 C2c。
...
| C2b3 | CatalogRevision/epoch 的定义、失效触发与重启语义；不得把 open/close 当作 Project 业务内容提交，不得用墙钟时间冒充单调版本 | 本地检查点通过：Debug 360/360、专项 13/13、13 项各 10 次和纯生产/架构通过，见 [C2b3 目录版本契约](ST1C2b3-目录版本与失效.md)；下一步 C2c，ST1D 仍未签核 |
