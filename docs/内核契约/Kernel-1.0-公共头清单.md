...
C6b21 已在实现基线 `9ea3429` 重新枚举当前目录：基线 71、当前 71、逐文件 SHA-256 不一致 0，并在 [71 公共头与兼容清单最终对账](ST1C6b21-71公共头与兼容清单最终对账.md) 为每个头落实语义、权限与后续节点。这关闭 C6b 的清单对账，不把文件数和摘要代替行为测试，也不表示 Kernel API 已最终冻结。后续任何公共头修改均需更新基线及兼容说明；最终冻结基线只能在 C6–C8/ST1D 验收后签发。当前执行顺序见 [C6 契约](ST1C6-公共契约与输入预算.md)。

## C6c1 公共头增量

C6c1 后公共头总数仍为 71。相对本页 C6 起始基线恰有 1 个预期摘要变化、0 个新增头和 0 个删除头：

| 公共头 | C6 起始 SHA-256 | C6c1 SHA-256 | 兼容说明 |
| --- | --- | --- | --- |
| [foundation/value.hpp](../../include/lasercnc/foundation/value.hpp) | `BE0A6B38F7624654DE3011755B0C371A48E2FBA341BA80405CD00EF93ECC30B0` | `80D58BA591CA66660D3418879F98BF61A6BB6A3AAEC5AE575A177EDFC7F77127` | 新增共享预算类型、常量、评估函数和违规名称；不改变原 `Value` 字段、变体或对象布局；属于源码/API 增量，不承诺跨工具链 ABI |

其余 70 个公共头继续与下表 C6 起始摘要一致。详细预算、错误码和拒绝语义见 [C6c1 契约](ST1C6c1-共享Value与序列化预算.md)。本节是过程增量账本，不覆盖下方起始清单，也不代替 C6–C8/ST1D 后的最终冻结基线。

## C6c2a 公共头增量

C6c2a 后公共头总数仍为 71。相对 C6 起始基线共有 2 个预期摘要变化、0 个新增头和 0 个删除头：C6c1 的 `foundation/value.hpp` 保持上一节摘要，本节点新增以下变化。

| 公共头 | C6 起始 SHA-256 | C6c2a SHA-256 | 兼容说明 |
| --- | --- | --- | --- |
| [foundation/strong_id.hpp](../../include/lasercnc/foundation/strong_id.hpp) | `6C1B8317EDD98026E562012759F16D4ED8DDC0BCBF96E78944C8B75D53CAAC25` | `7D0927713717AC3E5887800E00D8402EC351F233A852E2245E009A450E78FEC2` | 新增 4096 字节常量及严格 UTF-8 构造准入；`StrongId` 仍只保存 `std::string`，精确字节比较/hash 与对象布局不变；超长或畸形旧输入改为失败关闭 |

其余 69 个公共头继续与下表 C6 起始摘要一致。规则与旧持久材料边界见 [C6c2a 契约](ST1C6c2a-StrongId统一字节与编码准入.md)。该记录不承诺跨工具链 ABI，也不替代最终冻结基线。
