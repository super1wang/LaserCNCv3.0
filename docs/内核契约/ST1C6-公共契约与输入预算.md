...
| C6b | 逐公共入口/DTO/错误 cause/持久版本清单；配置期、执行期、诊断、生命周期与内部恢复的可达权限区分；明确源码兼容和回滚边界 | 已建立 [71 个公共头基线](Kernel-1.0-公共头清单.md) 和 [逐项审计账本](ST1C6b-公共契约逐项审计.md)，登记 Infrastructure/端口并扩充 Foundation/观察/消息声明；C6b1/b2 检查点保留，C6b3 Schema 未知类型修复通过 Debug/Release 选集各 147/147、ASan 同选集三次及生产/边界。下一步消息未知枚举/跨版本合并/订阅复用，其余行为/字段/格式仍待完成，不声称已冻结 |
...
C6b2 的日志预检枚举 base 与全部保留轮转目标，具有 O(N log N) 集合成本和逐目标元数据查询。锁定库允许的 200000 数值上限不是实测容量；C6c/C7 必须测量保留数、目录深度、已有文件规模与初始化耗时/内存，并据证据制定支持配置。Foundation 逐项账本登记的未知 SchemaKind 风险已在 C6b3 取得真实红灯并修复，规则与兼容性见 [根类型准入](ST1C6b3-Schema根类型准入.md)。下一步核实消息未知枚举/合并/订阅归属，再推进剩余声明和统一预算，不用单个枚举修复代签所有值/错误/Schema 契约。
