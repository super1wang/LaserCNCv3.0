...
| C6b | 逐公共入口/DTO/错误 cause/持久版本清单；配置期、执行期、诊断、生命周期与内部恢复的可达权限区分；明确源码兼容和回滚边界 | 已建立 [71 个公共头基线](Kernel-1.0-公共头清单.md)，逐声明/字段/格式审计待完成，不声称已冻结 |
...
C6b 的路径补查优先级：SQLite 的 UTF-8 转换遇到不可转换的原生 UTF-16 时是否逸出 Result；日志两个输出当前只以 lexically_normal() 比较，Windows 大小写/硬链接别名是否仍能指向同一文件。二者目前是代码审计风险项，须以独立回归和真实材料确认，不预先宣布缺陷或完成修复；如确认，必须在最终契约签核前处理。
