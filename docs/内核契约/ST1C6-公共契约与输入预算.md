...
| C6b | 逐公共入口/DTO/错误 cause/持久版本清单；配置期、执行期、诊断、生命周期与内部恢复的可达权限区分；明确源码兼容和回滚边界 | [71 公共头基线](Kernel-1.0-公共头清单.md) 与 [声明账本](ST1C6b-公共契约逐项审计.md) 保留；最新 [C6b16 快照存储写入证明准入](ST1C6b16-快照存储写入证明准入.md) 验证 SnapshotWriteDisposition 及 AlreadyPresent 内容，C6b15 版本规则保留。下一步补 Host、其余状态与持久族类型/字段/格式；不声称已冻结 |
