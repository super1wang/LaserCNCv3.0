...
| C6b | 逐公共入口/DTO/错误 cause/持久版本清单；配置期、执行期、诊断、生命周期与内部恢复的可达权限区分；明确源码兼容和回滚边界 | [71 公共头基线](Kernel-1.0-公共头清单.md) 与 [声明账本](ST1C6b-公共契约逐项审计.md) 保留；最新 [C6b10 Diagnostics 并发顺序](ST1C6b10-Diagnostics并发与latest顺序.md) 串行同一注册项至 latest 发布并拒绝递归自死锁。下一步观察 exporter 失败资源边界，再补其他类型/字段/格式；不声称已冻结 |
...
C6b4/b5 修复消息身份、版本和回调资源，C6b6 修复观察终态/聚合/诊断准入，C6b7 补日志出口，C6b8/b9 补 Trace 资源原子性，[C6b10](ST1C6b10-Diagnostics并发与latest顺序.md) 再补 Diagnostics 同 ID 串行/latest 顺序和递归拒绝；观察 exporter 失败资源边界为下一项。队列总量、失效候选保留、共享实例/回调原件及全队列复制成本继续由 C6c/d/C7 收口。
