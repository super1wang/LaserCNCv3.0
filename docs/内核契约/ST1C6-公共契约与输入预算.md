...
| C6b | 逐公共入口/DTO/错误 cause/持久版本清单；配置期、执行期、诊断、生命周期与内部恢复的可达权限区分；明确源码兼容和回滚边界 | [71 公共头基线](Kernel-1.0-公共头清单.md) 与 [声明账本](ST1C6b-公共契约逐项审计.md) 保留；最新 [C6b17 持久 DTO 写前准入](ST1C6b17-持久DTO写前准入.md) 关闭 Journal/Task/Workflow 的 15 个未知枚举和矛盾形状分支，C6b16 Snapshot 写入证明保留。下一步补 Host、State、Task Error cause 及其他持久类型/字段/格式；不声称已冻结 |
