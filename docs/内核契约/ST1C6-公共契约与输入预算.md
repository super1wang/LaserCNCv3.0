...
| C6b | 逐公共入口/DTO/错误 cause/持久版本清单；配置期、执行期、诊断、生命周期与内部恢复的可达权限区分；明确源码兼容和回滚边界 | [71 公共头基线](Kernel-1.0-公共头清单.md) 与 [声明账本](ST1C6b-公共契约逐项审计.md) 保留；最新 [C6b5 消息回调资源](ST1C6b5-消息回调资源与异常边界.md) 修复已通过 Debug 全集 449/449、Release 选集 172/172、ASan 同选集三次 516/516、探针另 3/3 及纯生产/源边界。下一步观察准入/终态、累计数值与资源边界；其他类型/字段/格式仍待审计，不声称已冻结 |
...
C6b4 已对消息身份/版本风险取得真实红灯并修复，见 [消息契约](ST1C6b4-消息准入与订阅身份.md)；[C6b5](ST1C6b5-消息回调资源与异常边界.md) 承接 callback 析构重入及复制异常，取得七项真实红灯，Debug 全集 449/449、Release 选集 172/172、ASan 同选集三次及生产/源边界通过。观察服务的同类资源路径和未知枚举/终态/累计数值为下一项。队列总量、失效候选保留、共享实例/回调原件及全队列复制成本继续由 C6c/d/C7 收口。
