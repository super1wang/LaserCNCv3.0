...
| C6b | 逐公共入口/DTO/错误 cause/持久版本清单；配置期、执行期、诊断、生命周期与内部恢复的可达权限区分；明确源码兼容和回滚边界 | [71 公共头基线](Kernel-1.0-公共头清单.md) 与 [声明账本](ST1C6b-公共契约逐项审计.md) 保留；最新 [C6b11 出口失败记录隔离](ST1C6b11-观察出口失败记录资源隔离.md) 保证失败诊断 OOM 不跳过后续 exporter 或反转本地结果。下一步 exporter 快照复制资源语义，再补其他类型/字段/格式；不声称已冻结 |
...
C6b4–b10 修复消息、观察资源及 Diagnostics 并发，[C6b11](ST1C6b11-观察出口失败记录资源隔离.md) 再隔离三服务 exporter 失败记录 OOM；exporter 快照复制资源语义为下一项。队列总量、失效候选保留、共享实例/回调原件及全队列复制成本继续由 C6c/d/C7 收口。
