...
| C6c | Value/Schema/序列化/配置与命令、查询、任务、工作流、脚本及恢复材料的统一尺寸、深度、数量和累计预算；输入/输出均检查，拒绝不留部分提交 | C6c1 与 [C6c2a](ST1C6c2a-StrongId统一字节与编码准入.md) 本地检查点通过：共享 Value/Schema/JSON/TOML 预算及全部 StrongId 的 4096 字节/严格 UTF-8 准入已实现。下一步 C6c2b 注册描述/定义，再完成 C6c2c 运行期请求/消息/观察和 C6c2d 持久/恢复 DTO；C6c 尚未整体签核 |
...
C6c1 先统一最底层可复用预算：`Value` 深度 64、节点 100000、对象键与字符串值累计 16 MiB，JSON/TOML 编码输入输出 64 MiB；限制只能收紧不能放宽。真实 Release 红灯确认 JSON、TOML 与 Schema 均会接纳第 65 层材料，修复后 Debug/Release 焦点各 8/8、ASan 三轮 24 次、Release 全量 498/498、纯生产和 71/141 边界通过，见 [C6c1 契约](ST1C6c1-共享Value与序列化预算.md)。`value.hpp` 是唯一有意公共头增量，原 Value 布局未变；C6c2 仍须在注册、排队、事务、发布或持久写入前覆盖执行 DTO、消息/观察、错误与持久材料的非 Value 文本、身份、集合和跨字段累计预算。

C6c2a 在全部 `StrongId<Tag>` 的唯一构造入口统一 4096 字节和严格 UTF-8 准入，保留合法精确字节、不做规范化或路径解释。真实 Release 红灯确认旧实现接纳 4097 字节身份；修复后的首次全量还发现快照测试仍假定超长身份可先构造，已改为验证更早拒绝和目录零副作用，最终 Release 499/499、Debug 焦点 2/2、ASan 三轮 6 次、纯生产及 71/141 边界通过，见 [C6c2a 契约](ST1C6c2a-StrongId统一字节与编码准入.md)。下一步按 C6c2b 注册定义 → C6c2c 运行期请求/消息/观察 → C6c2d 持久/恢复 DTO 推进。

### C6c2 剩余执行分解

| 子节点 | 必须覆盖 | 当前状态 |
| --- | --- | --- |
| C6c2a | 全部 StrongId 的统一字节与编码入口；与快照信封历史防线分层 | 本地检查点通过，不等于 C6c 完成 |
| C6c2b | Command/Query/Task/Workflow/Script 描述与定义的 Schema、Value、文本、依赖/资源集合及跨字段累计预算 | 下一活动节点 |
| C6c2c | 运行期请求/响应、DomainEvent 与 Trace/Metrics/Diagnostics/Log 的载荷、标签、消息及集合预算 | 待实现 |
| C6c2d | 事务、History、Snapshot、Idempotency、Task/Effect/Workflow/Diagnostic 与恢复 DTO 的写读双向累计预算 | 待实现 |
