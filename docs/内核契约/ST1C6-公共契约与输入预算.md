...
| C6b | 逐公共入口/DTO/错误 cause/持久版本清单；配置期、执行期、诊断、生命周期与内部恢复的可达权限区分；明确源码兼容和回滚边界 | 已建立 [71 个公共头基线](Kernel-1.0-公共头清单.md) 和 [逐项审计账本](ST1C6b-公共契约逐项审计.md)，登记 Infrastructure/端口并扩充 Foundation/观察/消息声明；C6b1 编码检查点保留，C6b2 日志身份/轮转通过最终 Debug 426/426、ASan 68 项各三次及生产/边界。优先核实 Schema/消息未知枚举等风险，其余行为/字段/格式仍待完成，不声称已冻结 |
...
C6b2 的日志预检枚举 base 与全部保留轮转目标，具有 O(N log N) 集合成本和逐目标元数据查询。锁定库允许的 200000 数值上限不是实测容量；C6c/C7 必须测量保留数、目录深度、已有文件规模与初始化耗时/内存，并据证据制定支持配置。Foundation 逐项账本已登记未知 SchemaKind 被弱化的待核实风险，下一子节点优先通过真实回归判断并修复，再推进其他声明和统一预算，不在日志验证中混入基础类型改动。
