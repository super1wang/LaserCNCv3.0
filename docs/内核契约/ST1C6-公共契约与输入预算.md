...
| C6b | 逐公共入口/DTO/错误 cause/持久版本清单；配置期、执行期、诊断、生命周期与内部恢复的可达权限区分；明确源码兼容和回滚边界 | [71 公共头基线](Kernel-1.0-公共头清单.md) 与 [声明账本](ST1C6b-公共契约逐项审计.md) 保留；最新 [C6b8 Trace 准入原子性](ST1C6b8-Trace准入原子性与身份保留.md) 修复资源失败孤儿并确认身份窗口/Core 寿命。下一步完成阶段资源失败与 Diagnostics 时间顺序，再补其他类型/字段/格式；不声称已冻结 |
...
C6b4 已对消息身份/版本风险取得真实红灯并修复，见 [消息契约](ST1C6b4-消息准入与订阅身份.md)；[C6b5](ST1C6b5-消息回调资源与异常边界.md) 承接 callback 析构重入及复制异常。[C6b6](ST1C6b6-观察状态与聚合准入.md) 修复观察终态、指标溢出及诊断状态/注册资源，[C6b7](ST1C6b7-日志出口与等级准入.md) 补独立日志出口和等级/负时间准入，[C6b8](ST1C6b8-Trace准入原子性与身份保留.md) 再补 Trace 准入原子性、身份窗口和 Core 寿命；完成阶段资源失败与 Diagnostics 顺序为下一项。队列总量、失效候选保留、共享实例/回调原件及全队列复制成本继续由 C6c/d/C7 收口。
