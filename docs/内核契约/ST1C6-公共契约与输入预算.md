...
| C6b | 逐公共入口/DTO/错误 cause/持久版本清单；配置期、执行期、诊断、生命周期与内部恢复的可达权限区分；明确源码兼容和回滚边界 | 已建立 [71 个公共头基线](Kernel-1.0-公共头清单.md) 和 [逐项审计账本](ST1C6b-公共契约逐项审计.md)，先登记 8 个基础设施类、13 个配置字段、6 个 platform 端口；C6b1 非法 UTF-16 修复通过相关 Debug 57/57、ASan 57 项各三次及纯生产/边界检查，其余声明/字段/格式待完成，不声称已冻结 |
...
C6b1 已实测确认 SQLite 不可转换的 UTF-16 会逸出 Result，其他文件适配器会接受并创建异常原生名称；已统一拒绝未配对代理码元，并使 Snapshot 错误诊断不再转换坏路径，具体兼容变化与证据见 [逐项账本](ST1C6b-公共契约逐项审计.md)。日志两个输出当前仍只以 lexically_normal() 比较，Windows 大小写/硬链接/父目录别名及轮转目标关系继续登记为待实测风险；不可预先写成通过，也不得仅为通过测试而全局禁止合法大小写敏感文件。
