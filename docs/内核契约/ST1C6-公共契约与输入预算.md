...
| C6b | 逐公共入口/DTO/错误 cause/持久版本清单；配置期、执行期、诊断、生命周期与内部恢复的可达权限区分；明确源码兼容和回滚边界 | [71 公共头基线](Kernel-1.0-公共头清单.md) 与 [声明账本](ST1C6b-公共契约逐项审计.md) 保留；最新 [C6b6 观察准入](ST1C6b6-观察状态与聚合准入.md) 通过 Debug/Release 选集各 181/181、ASan 同选集三次 543/543、探针另 3/3 和生产/源边界。下一步独立日志出口/等级，再完成其他类型/字段/格式；不声称已冻结 |
...
C6b4 已对消息身份/版本风险取得真实红灯并修复，见 [消息契约](ST1C6b4-消息准入与订阅身份.md)；[C6b5](ST1C6b5-消息回调资源与异常边界.md) 承接 callback 析构重入及复制异常，取得七项真实红灯，Debug 全集 449/449、Release 选集 172/172、ASan 同选集三次及生产/源边界通过。[C6b6](ST1C6b6-观察状态与聚合准入.md) 已取证修复观察终态、指标溢出及诊断状态/注册资源，独立日志出口与等级为下一项。队列总量、失效候选保留、共享实例/回调原件及全队列复制成本继续由 C6c/d/C7 收口。
