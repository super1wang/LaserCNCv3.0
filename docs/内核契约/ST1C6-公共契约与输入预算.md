...
| C6b | 逐公共入口/DTO/错误 cause/持久版本清单；配置期、执行期、诊断、生命周期与内部恢复的可达权限区分；明确源码兼容和回滚边界 | [71 公共头基线](Kernel-1.0-公共头清单.md) 与 [声明账本](ST1C6b-公共契约逐项审计.md) 保留；最新 C6b4 消息三类缺陷通过 Debug/Release 选集各 161/161、ASan 同选集三次及生产/边界。下一步 callback 析构重入及复制/异常边界，其他类型/字段/格式仍待审计，不声称已冻结 |
...
C6b4 已对上述消息风险取得五项真实红灯并修复，见 [消息契约](ST1C6b4-消息准入与订阅身份.md)。下一个必须项是 callback 捕获资源析构重入和用户定义复制/异常边界；源代码中 cancel、Bus 析构和投递快照涉及内部锁，不能仅以“回调调用在锁外”判定全部用户代码安全。队列总量、失效候选保留、共享实例标识和全队列复制成本继续由 C6c/d/C7 收口。
