...
| C6b | 逐公共入口/DTO/错误 cause/持久版本清单；配置期、执行期、诊断、生命周期与内部恢复的可达权限区分；明确源码兼容和回滚边界 | [71 公共头基线](Kernel-1.0-公共头清单.md) 与 [声明账本](ST1C6b-公共契约逐项审计.md) 保留；最新 [C6b13 编排定义枚举准入](ST1C6b13-编排定义枚举准入.md) 拒绝 Workflow/Script 未知步骤、节点、谓词与等待目标，C6b12 出口快照规则保留。下一步补 Host/执行/状态/持久族类型/字段/格式；不声称已冻结 |
...
C6b4–b10 修复消息、观察资源及 Diagnostics 并发，[C6b11](ST1C6b11-观察出口失败记录资源隔离.md) 隔离三服务 exporter 失败记录 OOM，[C6b12](ST1C6b12-观察出口快照无分配发布.md) 消除完整 exporter 向量复制分配，[C6b13](ST1C6b13-编排定义枚举准入.md) 再补 Workflow/Script 结构化定义枚举准入。队列总量、失效候选保留、共享实例/回调原件、观察与编排内容成本继续由 C6c/d/C7 收口。
