...
- [LocalTraceService](../../src/runtime/observability/local_trace_service.cpp) 的 Running/未定义 end 已在 C6b6 归一为带诊断的 Failed，覆盖 active、records、exporter、重复/析构与双线程单次完成；资源耗尽下的活动/记录一致性和身份保留仍待核验。
- [LocalMetricsService](../../src/runtime/observability/local_metrics_service.cpp) 的有限值累加溢出已在 C6b6 通过公开三入口负例修复；count=max 由共用私有算术回归，不冒充海量公共调用。输入字节/序列总量、时间戳与导出资源异常仍待预算审计。
- [DiagnosticsService](../../src/runtime/observability/diagnostics_service.cpp) 的未知 status 和重复注册临时 Entry 锁内析构已在 C6b6 取证修复；合法 Unknown、冻结拒绝及原检查保留。latest 时间顺序、并发 Check 及全局资源预算仍待核验。
...
1. C6b2–b5 检查点保留；C6b6 修复观察终态、聚合溢出、诊断状态及注册资源，见 [交付](../阶段交付/2026-09-05-ST1C6b6-观察状态与聚合准入.md)。下一步优先核实直接 LogObservabilityExporter 的 DTO/异常/时间差和 SpdlogLogService 未知等级准入；随后完成其余公共类型、时间顺序/身份/资源失败，不以 Local 服务修复代签独立出口。
