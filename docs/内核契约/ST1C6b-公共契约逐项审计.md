...
- [LocalTraceService](../../src/runtime/observability/local_trace_service.cpp) 的 Running/未定义 end 已在 C6b6 归一；C6b8 修复准入分配失败遗留孤儿，确认窗口内防重、淘汰后复用及句柄晚于服务所有者。完成阶段分配失败、活动总量仍待核验。
...
1. C6b2–b7 检查点保留；C6b8 修复 Trace 准入分配失败孤儿并确认窗口身份与 Core 寿命，见 [交付](../阶段交付/2026-09-05-ST1C6b8-Trace准入原子性与身份保留.md)。下一步完成阶段资源失败、Diagnostics latest 时间顺序，然后补 Host/执行/状态/持久族 DTO 与格式；不以 Trace 局部修复代签其他入口。
