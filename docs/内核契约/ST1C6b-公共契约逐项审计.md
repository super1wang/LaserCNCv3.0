...
- [DiagnosticsService](../../src/runtime/observability/diagnostics_service.cpp) 的未知 status/重复注册已在 C6b6 修复；C6b10 明确同 ID 串行至本地发布、不同 ID 并行、递归拒绝和 latest 顺序。exporter 失败记录资源及全局预算仍待核验。
...
1. C6b2–b9 检查点保留；C6b10 修复 Diagnostics 同 ID 并发覆盖和递归自死锁，明确 latest 本地发布顺序，见 [交付](../阶段交付/2026-09-05-ST1C6b10-Diagnostics并发与latest顺序.md)。下一步观察 exporter 失败资源边界，然后补 Host/执行/状态/持久族 DTO 与格式；不以 Diagnostics 局部修复代签其他入口。
