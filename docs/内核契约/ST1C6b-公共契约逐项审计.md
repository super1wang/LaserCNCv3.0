...
- [LocalMetricsService](../../src/runtime/observability/local_metrics_service.cpp) 的聚合溢出已在 C6b6 修复；C6b11 隔离 exporter 失败诊断 OOM。输入字节/序列总量、时间戳与 exporter 快照资源异常仍待审计。
- [DiagnosticsService](../../src/runtime/observability/diagnostics_service.cpp) 的状态/注册由 C6b6 修复，并发/latest 由 C6b10 修复；C6b11 隔离 exporter 失败诊断 OOM。快照复制和全局预算仍待核验。
...
1. C6b2–b10 检查点保留；C6b11 隔离三服务 exporter 失败记录 OOM，见 [交付](../阶段交付/2026-09-05-ST1C6b11-观察出口失败记录资源隔离.md)。下一步 exporter 快照复制资源语义，然后补 Host/执行/状态/持久族 DTO 与格式；不以观察局部修复代签其他入口。
