...
C2b2b 后续补充：终态 Workflow 历史已按认证持久目录恢复，不要求当前文档 Open，不重开容器；非终态准入不变。Debug 328/328、14 项各 10 次与纯生产通过，详见 [C2b2b 交付](../阶段交付/2026-09-04-ST1C2b2b-终态工作流历史恢复.md)。生命周期命令分类及其他 legacy 身份范围仍须继续验收。
...
C2b2d 本地检查点通过：历史执行根认证、一次性迁移及后续缺根拒绝已完成，最终 Debug 347/347、专项 8/8、新增 8 项各 10 次与纯生产/架构通过；见 [C2b2d 交付](../阶段交付/2026-09-04-ST1C2b2d-历史执行项目根认证.md)。上述为各节点历史证据，当前下一步 C2b3。认证不等于恢复执行，C2b3 目录失效与 C2c drain/析构仍独立保留。

C2b3 本地检查点通过：独立目录快照与失效契约已实现，最终 Debug 360/360、专项 13/13（含实际同库三进程重启）、13 项各 10 次及纯生产/架构通过，见 [C2b3 交付](../阶段交付/2026-09-04-ST1C2b3-目录版本与失效.md)。目录令牌不扩大执行权限，也不替代 Host 生命周期；下一步 C2c 继续核验真实 drain、超时和析构依赖保留，C2 与 ST1D 尚未最终签核。

## C2c 下一步核验顺序

2026-09-04 开始实施，详细状态见 [C2c 子节点契约](ST1C2c-停止确认与析构依赖.md)。C2c1 已修复下述第 3 项停止确认/重试漏洞，并将终态发布纳入成功前提；Debug 368/368、专项 8/8、停止相关 10 项各 10 次和纯生产/架构通过，见 [C2c1 交付](../阶段交付/2026-09-04-ST1C2c1-停止确认与失败重试.md)。C2c2 继续覆盖线程、析构和所有权联动。拆分只用于保留可验证提交，不缩小下面的验收要求。

1. 明确 Host 生命周期线程及对象存续责任。公开调用与析构并发不因状态字段原子化而自动合法；从自己的执行器工作线程同步 drain 必须有明确拒绝/前置条件，不能自等待。
2. 先验证当前 AppKernel 析构只尝试 shutdown 后直接进入成员逆序销毁的路径：非协作 Task 超时后，其依赖及 completion/持久观察回调必须仍存活，不能依靠更晚销毁的执行器补救已经释放的 Runtime。
3. 核验 ITaskExecutor 的 waitIdle/shutdown/析构保证、失败和异常语义。Scheduler 在执行器 shutdown 前清除 started 的重试路径须专项覆盖，不能因一次失败就把仍需 drain 的执行器当作已停止。
4. 用可控执行器和真实线程池分别覆盖 Ready/Stopping/Configuring/Failed、超时后完成及重试、模块停止失败、终态回调尾部和析构；必要的崩溃/超时证据放在独立测试进程，不让测试超时被误记为成功。
5. 完成上述正确性实现和门禁后再确定 C2 完整检查点；不得从 C2b 目录令牌、活动计数或最终值全绿推导任意析构安全。C3/C4/C6–C8/ST1D 仍独立保留。
