...
以上是 E2A 提交时的边界；后续状态准入已由 [E2B](K10E-E2B-资产引用状态准入.md) 闭合，本文的 192 项证据保持为组件节点历史记录。
...
ST1C3b 在该文件适配器加入 `LCNCSN02` 外层身份信封与 `@SHA256.snapshot` 文件键；上述 `LCNCAS01` 资产信封作为原始负载保持不变，AssetRef 不升级。旧资产文件精确大小写兼容，新旧双格式歧义拒绝；新文件不能用旧二进制直接读取，回滚需匹配整套持久材料。当前验收进度见 [C3 契约](ST1C3-快照身份与存储键.md)，不将历史 E2A 成绩自动视为新存储格式验收。
