...
C6b4 源码复核发现的 cancel/Bus 析构锁内销毁 callback，以及复制/分配异常边界，由 [C6b5](ST1C6b5-消息回调资源与异常边界.md) 接续取得回归并修复。C6b4 自身测试不证明该项通过；新规则保留已取得的回调原件快照，取消后捕获资源可能延迟销毁，不把取消等同于 join。
