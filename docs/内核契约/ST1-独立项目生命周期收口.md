...
2026-09-04 交叉审计后，剩余顺序及独立缺陷门禁以 [ST1C 补充执行计划](ST1C-补充审计与剩余执行计划.md) 为准。ST1C 不再仅指项目存在性租约，还包括独立修订恢复、Kernel 停止/析构、存储键、可信装载、单 Host 与冻结契约。C1a 非零 ProjectRevision 独立恢复已通过本地检查点，证据见 [C1a 交付](../阶段交付/2026-09-04-ST1C1a-项目修订独立恢复.md)；下一步 C5/C1b，全部必须项闭合后进入 ST1D。
...
迁移已经完成时，completeProjectCatalogMigration 是不添加身份的幂等空操作，不表示其参数已经获准创建项目。每次 Kernel 启动仍必须把全部已验证持久所属身份与现有项目目录逐一核对，缺失时拒绝，不能用 saveProjectLifecycle 补造。ST1A 阶段迁移源只承接真实文档根身份；C2b2d 已在此基础上明确接入受支持 Task/Effect 历史的摘要、帧与归属认证，并在迁移写事务内二次核验。只有认证后的真实请求归属可以补入 pending 迁移集合，不能据任意请求字符串自动创造项目；已完成迁移后的缺根仍拒绝。实现及门禁进展见 [C2b2d 交付](../阶段交付/2026-09-04-ST1C2b2d-历史执行项目根认证.md)，不由历史认证授予执行权。
