...
以下表格列出逐项实现证据及签核边界；通过不等于任意输入、线程交错、同进程恶意代码或物理硬件的数学保证。EX2 的 Project scope 当前按 K10A 的上下文形状及文档所属关系核验，不能据此推导 ST1 中独立项目的存在性和生命周期已经实现。

补充发现及修复：Host 可变 History/Persistence 的底层状态写入口已在 F5B 封闭；Scheduler 只读入口、未知 ContractStatus 拒绝和 scope 直接断言在 F5C 补齐。本轮三配置与连续矩阵验证后签核相关项目；Project 范围确认独立保留，见 [F5B 审计](K10F-F5B-Host状态写入口审计.md) 与 [F5C 终审](K10F-F5C-执行边界与状态终审.md)。
