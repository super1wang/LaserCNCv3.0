...
2026-09-04 用户已确认按交叉审计补齐计划。当前状态见 [ST1C 补充执行计划](ST1C-补充审计与剩余执行计划.md)：最新 C6b13 在 Workflow/Script 组合注册期显式拒绝未知步骤、节点、谓词与等待目标，见 [交付](../阶段交付/2026-09-05-ST1C6b13-编排定义枚举准入.md)；C6b12 的观察出口快照规则继续保留。下一步继续 Host/执行/状态/持久族类型与格式；C6b/c/d、C7/C8、ST1D 仍未签核。历史检查点不自动覆盖新增实现，公共头/声明登记不是完整冻结，历史 31/32 不表示只剩一项。
