...
2026-09-04 用户已确认按交叉审计补齐计划。当前状态见 [ST1C 补充执行计划](ST1C-补充审计与剩余执行计划.md)：最新 C6b15 使 Command/Query 的 Registry 与 Runtime 显式拒绝未知 VersionResolution，并修正失败 Trace 的策略名称，见 [交付](../阶段交付/2026-09-05-ST1C6b15-版本解析策略枚举准入.md)；C6b14 的资源声明规则继续保留。下一步继续 Host、状态与持久族类型/格式；C6b/c/d、C7/C8、ST1D 仍未签核。历史检查点不自动覆盖新增实现，公共头/声明登记不是完整冻结，历史 31/32 不表示只剩一项。
