...
当前收口按 [ST1C 补充执行计划](docs/内核契约/ST1C-补充审计与剩余执行计划.md) 继续：最新 [C6b4 消息准入与订阅身份](docs/阶段交付/2026-09-05-ST1C6b4-消息准入与订阅身份.md) 已修复未知枚举、Notification 跨版本合并及同名订阅复用错投。Debug/Release 选集各 161/161、ASan 同选集各三次 483/483、探针另 3/3 和纯生产/边界通过，不宣称全量 438 项。下一步按 [C6 子节点](docs/内核契约/ST1C6-公共契约与输入预算.md) 核实回调捕获资源析构重入和用户定义复制/异常边界，再继续其他公共契约、统一预算及终态保留，之后 C7/C8 与 ST1D。C1–C5 和 C6 既有检查点保留其受测范围，整体尚未 Frozen，不扩展上层模块。
