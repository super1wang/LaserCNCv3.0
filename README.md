...
当前收口按 [ST1C 补充执行计划](docs/内核契约/ST1C-补充审计与剩余执行计划.md) 继续：C1–C4 正确性及 C5 支持矩阵/独占/离线恢复已有检查点；最新 [C6a 文件路径与 Unicode 日志](docs/阶段交付/2026-09-04-ST1C6a-文件路径精确性与Unicode日志.md) 修复五类路径选项的 NUL 截断及日志文件名乱码，全量 Debug 412/412、定向 ASan 54 项各 3 次、纯生产/架构通过。下一步按 [C6 子节点](docs/内核契约/ST1C6-公共契约与输入预算.md) 完成逐入口契约、统一预算和终态保留，再推进 C7/C8 与 ST1D；71 个公共头清单仅是差异基线，未验证环境和最终签核明确保留。下文按节点保留历史范围，旧门禁不自动覆盖新增代码。
