...
当前收口按 [ST1C 补充执行计划](docs/内核契约/ST1C-补充审计与剩余执行计划.md) 继续：最新 [C6b21 最终对账](docs/阶段交付/2026-09-05-ST1C6b21-71公共头与兼容清单最终对账.md) 已确认当前 71 个公共头与 C6a 基线逐文件摘要一致，完成 C6b1–b20 的声明、权限、51 个新增/变更错误码及持久格式兼容矩阵对账。下一步按 [C6 子节点](docs/内核契约/ST1C6-公共契约与输入预算.md) 进入 C6c 统一预算，再完成 C6d 同步/Task/终态保留，之后 C7/C8 与 ST1D。整体尚未 Frozen，不扩展上层模块。
