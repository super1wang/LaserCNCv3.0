...
当前收口按 [ST1C 补充执行计划](docs/内核契约/ST1C-补充审计与剩余执行计划.md) 继续：C1–C4 正确性及 C5 支持矩阵/独占/离线恢复已有检查点；最新 [C6b1 路径编码与契约登记](docs/阶段交付/2026-09-04-ST1C6b1-路径编码准入与契约登记.md) 修复非法 UTF-16 逸出 Result/创建输出及错误诊断问题，相关 Debug 57/57、ASan 57 项各三次、纯生产/边界通过。此前 C6a 全量 Debug 412/412 保留受测版本范围，不代签新增代码。下一步按 [C6 子节点](docs/内核契约/ST1C6-公共契约与输入预算.md) 继续日志别名和逐入口契约、统一预算、终态保留，再推进 C7/C8 与 ST1D；公共头和配置字段登记不是全契约冻结，未验证环境和最终签核明确保留。下文按节点保留历史范围，旧门禁不自动覆盖新增代码。
